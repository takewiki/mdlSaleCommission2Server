...
promotionEmp_Query <- function(dms_token='9B6F803F-9D37-41A2-BDA0-70A7179AF0F3',
  ...
  sql <- paste0("SELECT
     ...
     FROM  rds_hr_sales_promotion_WithCompare_mfg
...
promotionEmp_server <- function(input,output,session,dms_token) {
  ...
  var_cp_pfm_emp_org <- tsui::var_ListChoose1('cp_pfm_emp_mfg_org')

  var_cp_pfm_emp_YearPeriod <- tsui::var_text('cp_pfm_emp_mfg_YearPeriod')




  shiny::observeEvent(input$cp_pfm_emp_mfg_query,{
    ...
    #print(data)
    tsui::run_dataTable2(id = 'cp_pfm_emp_mfg_dataView',data = data)
    file_name = paste('提成汇总',FSaleOrgName,FYearPeriod,'.xlsx',sep = '_')
    tsui::run_download_xlsx(id = 'cp_pfm_emp_mfg_download',data = data,filename =file_name )
