...
sql <- paste0("SELECT
      ...
      ,[FNote]  as 备注
  FROM [cprds].[dbo].[rds_hr_sales_promotion_WithCompare_mfg]
...
promotion_server <- function(input,output,session,dms_token) {
  ...
  #提成查询
  var_cp_pfm_res_org <-tsui::var_ListChoose1('cp_pfm_res_mfg_org')
  #暂估使用新账套

  var_cp_pfm_res_db <- tsui::var_ListChoose1('cp_pfm_res_mfg_db')

  var_cp_pfm_res_dateRange_query <-tsui::var_dateRange('cp_pfm_res_dateRange_mfg_query')


  shiny::observeEvent(input$cp_pfm_res_mfg_query,{

    dates = var_cp_pfm_res_dateRange_query()
    FStartDate = dates[1]
    FEndDate = dates[2]
    FSaleManName = input$cp_pfm_res_saleMan_mfg_selector
    ...
    tsui::run_dataTable2(id = 'cp_pfm_res_mfg_dataView',data = data)
    file_name = paste('提成明细',FSaleOrgName,FSaleManName,FStartDate,FEndDate,'.xlsx',sep = '_')
    tsui::run_download_xlsx(id = 'cp_pfm_res_mfg_download',data = data,filename =file_name )
